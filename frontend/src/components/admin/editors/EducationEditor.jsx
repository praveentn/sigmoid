...
function EducationForm({ initial, onSave, saving, onClose }) {
  const [form, setForm] = useState({ institution: '', degree: '', year: '', description: '', order: 0, ...initial })
  const s = (key) => (e) => setForm(f => ({ ...f, [key]: e.target.value }))

  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form) }} className="space-y-0">
      <FormField label="Institution"><input className="admin-input" required value={form.institution} onChange={s('institution')} /></FormField>
      <FormField label="Degree"><input className="admin-input" required value={form.degree} onChange={s('degree')} /></FormField>
...
export default function EducationEditor() {
  const { items, loading, editing, saving, toast, setToast, openNew, openEdit, closeModal, handleSave, handleDelete } =
    useListEditor(api.getEducation, api.createEducation, api.updateEducation, api.deleteEducation)
  ...
  return (
    <div>
        ...
        {items.map(item => (
          <div key={item.id} className="bg-white border border-cloud px-5 py-4 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-silver">{item.institution} · {item.year}</p>
            </div>
      ...
      {editing !== null && (
        <Modal title={editing.id ? 'Edit Education' : 'Add Education'} onClose={closeModal}>
          <EducationForm initial={editing} onSave={handleSave} saving={saving} onClose={closeModal} />
        </Modal>
      )}
