...
function AwardForm({ initial, onSave, saving, onClose }) {
  const [form, setForm] = useState({ title: '', description: '', year: '', organization: '', order: 0, ...initial })
  const s = (key) => (e) => setForm(f => ({ ...f, [key]: e.target.value }))

  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form) }} className="space-y-0">
      <FormField label="Award Title"><input className="admin-input" required value={form.title} onChange={s('title')} /></FormField>
      <div className="grid grid-cols-3 gap-3">
        <FormField label="Organization"><input className="admin-input" value={form.organization} onChange={s('organization')} /></FormField>
...
export default function AwardsEditor() {
  const { items, loading, editing, saving, toast, setToast, openNew, openEdit, closeModal, handleSave, handleDelete } =
    useListEditor(api.getAwards, api.createAward, api.updateAward, api.deleteAward)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-xs text-silver font-mono">{items.length} awards</p>
        ...
        {items.map(item => (
          <div key={item.id} className="bg-white border border-cloud px-5 py-4 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-silver">{item.organization} · {item.year}</p>
            </div>
      ...
      {editing !== null && (
        <Modal title={editing.id ? 'Edit Award' : 'Add Award'} onClose={closeModal}>
          <AwardForm initial={editing} onSave={handleSave} saving={saving} onClose={closeModal} />
        </Modal>
      )}
